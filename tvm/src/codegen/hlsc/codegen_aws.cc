...
void CodeGenAWSHLS::AddFunction(LoweredFunc f,
  ...
  CodeGenHLSC::AddFunctionAWS(f, map_arg_type);
  if (soda_header_.is_open())
    soda_header_.close();
}

void CodeGenAWSHLS::PrintType(Type t, std::ostream& os) {
...
void CodeGenAWSHLS::VisitExpr_(const GetSlice* op, std::ostream& os) {
  PrintExpr(op->a, os);
  os << "(";
...
void CodeGenAWSHLS::VisitStmt_(const Store* op) {
  // handle SetSlice
...
void CodeGenAWSHLS::VisitStmt_(const For* op) {
...
void CodeGenAWSHLS::VisitStmt_(const Partition* op) {
...
void CodeGenAWSHLS::VisitStmt_(const Stencil* op) {
